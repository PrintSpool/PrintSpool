...
use crate::components::TemperatureHistoryEntry;
use crate::task::{self, Task, TaskStatus};
use crate::{
    machine::{
        self, events::TaskSettled, Errored, GCodeHistoryDirection, GCodeHistoryEntry, Machine,
        Machine, MachineStatus, PositioningUnits, Printing,
    },
    task::Created,
};
...
pub async fn update_tasks(
    ...
    if !machine.has_received_feedback {
        let mut tasks = Task::tasks_running_on_machine(db, &machine.get_data()?.config.id).await?;
        ...
        for mut task in tasks.iter_mut() {
            ...
            if !feedback.task_progress.iter().any(|p| p.task_id == task.id) {
                ...
                task.settle_task(tx, &machine_hooks, machine.data_ref()?, &ctx.address())
                    .await?;
            } else if
            // If the task is still present upon reconnection then update the printer status
            task.status.is_pending() && task.is_print() {
                info!("Reconnected and continuing Print #{}", task.id);
                machine.get_data()?.status = MachineStatus::Printing(Printing {
    ...
    for progress in feedback.task_progress.iter() {
        ...
        let mut task = if let Ok(task) = Task::get(db, &progress.task_id, true).await {
            task
        } else {
            warn!(
        ...
        let status_changed = if !task.status.is_settled()
        ...
        if status_changed && task.status.is_settled() {
            ...
            match &machine.get_data()?.status {
                MachineStatus::Printing(Printing {
                    ...
                    let next_task = Task::tasks_running_on_machine(db, &machine.id)
                        .await?
                        .into_iter()
                        .filter(|t| t.is_print())
                        .next();
        ...
        if status_changed && (task.status.is_settled() || task.status.is_paused()) {
            machine
                .send_message(DeleteTaskHistory { task_id: task.id }.into())
                .await?;
        }
    }
    ...
    if let Some(blocked_at) = machine.data.as_ref().and_then(|m| m.blocked_at) {
        if !is_blocking {
            let pending_tasks = Task::tasks_running_on_machine(db, &machine.id).await?;
...
pub async fn update_heaters(
    ...
    for h in feedback.heaters.iter() {
        ...
        if history
            .back()
            .map(|last| *now > last.created_at + Duration::milliseconds(500))
            .unwrap_or(true)
        {
            history.push_back(TemperatureHistoryEntry {
...
pub async fn update_machine(
    ...
    if next_status != machine_data.status && !next_status.is_driver_ready() {
        ...
        // to an errored state.
        let tasks = Task::tasks_running_on_machine(db, &machine_data.config.id).await?;
    ...
    if machine_data.status != next_status
    ...
    if let Some(flags) = MachineFlags::from_bits(feedback.machine_flags) {
        ...
        warn!(
            "Unable to parse machine flags: {:#b}",
            feedback.machine_flags
        );
    }
    ...
    for entry in feedback.gcode_history.iter() {
        let direction = if entry.direction == machine_message::GCodeHistoryDirection::Rx as i32 {
            GCodeHistoryDirection::Rx
        } else {
            GCodeHistoryDirection::Tx
        };

        history.push_back(GCodeHistoryEntry::new(entry.content.clone(), direction));
