...
use xactor::{Context as XContext, StreamHandler};
...
impl StreamHandler<RxResult> for Machine {
    ...
    async fn handle(&mut self, ctx: &mut XContext<Self>, msg: RxResult) {
        ...
        use machine_message::Payload::{Feedback, Init};

        let result = match msg.payload {
            Some(Init(init)) => record_init(self, init, ctx).await,
            Some(Feedback(feedback)) => record_feedback(self, feedback, ctx).await,
            _ => return,
        };
    ...
    async fn finished(&mut self, ctx: &mut XContext<Self>) {
        ...
        let shutdown_result = self
            .unix_socket
            .as_ref()
            .map(|socket| socket.shutdown(async_std::net::Shutdown::Both));
