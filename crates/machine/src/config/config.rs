...
impl Config {
        ...
        self.components
            ...
            .find(|component| {
                match component {
                    ...
                    Component::Axis(c) => c.model.address == address,
                    Component::Toolhead(c) => c.model.address == address,
                    Component::SpeedController(c) => c.model.address == address,
                    Component::Video(_) => false,
                    Component::BuildPlatform(c) => c.model.address == address,
                }
            })
    }
    ...
    pub fn feedrates(&self) -> impl std::iter::Iterator<Item = Feedrate> {
        self.components.clone().into_iter().filter_map(|component| {
            match component {
                Component::Axis( Axis { model: axis, .. } ) => Some(Feedrate {
                    address: axis.address.clone(),
                    feedrate: axis.feedrate,
                    reverse_direction: axis.reverse_direction,
                    is_toolhead: false,
                }),
                Component::Toolhead( Toolhead { model: toolhead, .. } ) => Some(Feedrate {
    ...
    pub fn heater_addresses(&self) -> Vec<String> {
        self.components
            ...
            .filter_map(|component| {
                match component {
                    | Component::BuildPlatform(BuildPlatform {
                        model: BuildPlatformConfig { heater: true, address, .. },
                    .. })
                    | Component::Toolhead(Toolhead {
                        model: ToolheadConfig { heater: true, address, .. },
                    .. }) => {
                        Some(address.clone())
                    }
                    _ => None
    ...
    pub fn fan_addresses(&self) -> Vec<String> {
        self.components
            ...
            .filter_map(|component| {
                match component {
                    | Component::SpeedController(SpeedController {
                        model: SpeedControllerConfig { address, .. },
                    .. }) => {
                        Some(address.clone())
                    }
                    _ => None
