 ...
 * the spooled line.
 */
const despoolToSerialLoop = (state, action) => {
  const { type } = action

  if (
    (type === DESPOOL_TASK && getCurrentLine(state) != null) ||
    (type === SPOOL_TASK && shouldSendSpooledLineToPrinter(state))
  ) {
    const lineNumber = getCurrentSerialLineNumber(state)
